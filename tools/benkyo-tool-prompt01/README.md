...
上部の「問題セット」コンボボックスと「ページ」コンボボックスは `src/data/index.json` を起点に全 dataset を読み、ページを横断して選べるようにしています。解答欄はテキスト入力でき、選択問題はその場でチェックやラジオ選択ができます。新しい問題JSONを追加したら、`src/data/` で次を実行してください。
